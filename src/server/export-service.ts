...
function renderLatex(canvas: CanvasDetail, solutions: Solution[], user: UserAccount, watermark: boolean) {
  const lines = [
    ...
    ...solutions.flatMap((solution, index) => [
      ...
      "\\textbf{Problem:}",
      "\\begin{equation*}",
      sanitizeTexMath(solution.problemText),
      "\\end{equation*}",
      "",
      "\\textbf{Steps:}",
      "\\begin{align*}",
      ...solution.steps.map((step) => `  & ${sanitizeTexMath(step.latex)} && \\text{${escapeLatex(step.explanation)}} \\\\`),
      "\\end{align*}",
      "",
      "\\textbf{Final Answer:}",
      "\\begin{equation*}",
      sanitizeTexMath(solution.finalAnswer),
      "\\end{equation*}",
...
// problemText / finalAnswer / step.latex are LaTeX math, so they can't be fully
// escaped without breaking rendering — but they're AI-generated and thus
// promptable by whoever drew the problem (and copied verbatim on remix). Strip
// the TeX primitives that read files or run shells so a downloaded .tex can't
// become file exfiltration (\input{/etc/passwd}) or RCE (\write18) when the
// recipient compiles it. Math control words (\frac, \int, \alpha, ...) survive.
// (?![a-zA-Z]) — a TeX control word is letters-only, so this matches the whole
// word (\write18, \input{...}) without firing on longer names like \writexyz.
const dangerousTexCommand =
  /\\(write|immediate|openout|closeout|openin|closein|read|input|include|includeonly|inputiffileexists|catcode|csname|endcsname|def|edef|gdef|xdef|let|futurelet|special|directlua|latelua|usepackage|loop)(?![a-zA-Z])/gi;

function sanitizeTexMath(value: string) {
  // Drop the leading backslash so the command degrades to inert visible text.
  return value.replace(dangerousTexCommand, (match) => match.slice(1));
}
