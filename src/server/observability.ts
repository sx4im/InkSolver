...
export async function getObservabilitySummary(limit = 500) {
  ...
  return {
    ...
    solve: {
      ...
      latestMs: latestSolveDurationMs(solveEvents),
    },
    cost: {
...
// solveEvents is ordered newest-first, so the first finite duration is the most
// recent solve — not the slowest, which sorting + .at(-1) would have surfaced.
function latestSolveDurationMs(solveEvents: UsageEvent[]) {
  for (const event of solveEvents) {
