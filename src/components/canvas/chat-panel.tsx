...
export function ChatPanel({
  ...
  // Detect Web Speech API after mount (SSR-safe: starts true, matches server).
  const [voiceSupported, setVoiceSupported] = useState(true);
  const scrollRef = useRef<HTMLDivElement | null>(null);
  const recognitionRef = useRef<{ stop: () => void } | null>(null);
  const formRef = useRef<HTMLFormElement | null>(null);
  const chatAbortRef = useRef<AbortController | null>(null);
  const chatTargetIdRef = useRef<string | null>(null);
  ...
  // Drop in-flight token writes when the user switches solution versions.
  useEffect(() => {
    chatAbortRef.current?.abort();
    chatAbortRef.current = null;
    chatTargetIdRef.current = solution?.id ?? null;
    setIsSending(false);
  }, [solution?.id]);
  ...
  const toggleListening = () => {
    ...
    if (!SpeechRecognition) return; // Button is disabled when unsupported.
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    chatTargetIdRef.current = targetSolutionId;

    let latestMessages = [...messages, userMessage, assistantMessage];
    onMessagesChange(latestMessages);

    const stillOnTarget = () => chatTargetIdRef.current === targetSolutionId && !abort.signal.aborted;
    ...
    try {
      ...
      if (abort.signal.aborted || !stillOnTarget()) return;
      latestMessages = latestMessages.map((item) =>
      ...
      if (stillOnTarget()) setIsSending(false);
    }
  }
  ...
  return (
    <>
      ...
      <div className="border-b border-hairline p-4">
        ...
        {focusedStep ? (
          <div className="mt-3 flex items-center justify-between gap-2 rounded-sm border border-primary/30 bg-canvas px-3 py-2 text-xs text-muted">
      ...
      <form ref={formRef} className="border-t border-hairline p-4" onSubmit={handleSubmit}>
        ...
        <div className="flex gap-2">
          ...
          <Button
            ...
            disabled={!solution || isSending || !voiceSupported}
