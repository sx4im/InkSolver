...
<!-- DESIGN.md removed — was an Airtable style reference used during initial development, not a living spec -->
This plan is derived from `01-InkSolver.md`. The project is intentionally split into more phases than the PRD sprint plan so each layer can be verified before the next one depends on it.
...
- UI system: white canvas, near-black ink actions, editorial spacing, restrained type, signature coral/forest/dark surfaces, small radii, minimal shadow.
...
- Editorial visual treatment aligned to the canvas/ink/signature-surface system.
